...
100명, 1000명의 user가 동시접속 할 때를 가정하여 송금하기와 계좌전체조회하기 API에서 성능테스트를 진행했습니다.
...
초당 요청수(RPS)는 약 35에 도달했습니다. 100명까지는 요청에 대한 실패율이 거의 없으며, 응답시간이 약 30ms에서 안정적으로 유지됩니다.
...
초당 요청수(RPS)는 약 160에 도달했고, 유저가 6~700명을 넘어갈 때부터 실패율이 계속해서 증가합니다. 
...
서버가 100명일 때의 35RPS보다 약 4.5배 더 증가한 160 RPS를 처리하면서, 실패율이 급격하게 상승했습니다. 700명 이상일때부터는 급격하게 실패율이 증가하며 성능이 저하됩니다. 
